...
id3v2AddLameVersion(lame_global_flags * gfp)
{
    ...
    if (lenb > 0) {
        sprintf_s(buffer, sizeof(buffer), "LAME %s version %s (%s)", b, v, u);
    }
    else {
        sprintf_s(buffer, sizeof(buffer), "LAME version %s (%s)", v, u);
    }
    ...
    if (fieldvalue && *fieldvalue) {
        ...
        if (frame_id != 0) {
            if (id3tag_set_textinfo_latin1(gfp, fieldvalue, &fieldvalue[5])) {
                ...
                gfc->tag_spec.values[gfc->tag_spec.num_values++] = _strdup(fieldvalue);
            }
        }
        gfc->tag_spec.flags |= CHANGED_FLAG;
    }
